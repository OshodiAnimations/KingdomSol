...
export function LoadingScreen() {
  ...
  const handleTap = () => {
    ...
    // If profile exists go to menu, otherwise create profile first
    setScreen(profile ? 'menu' : 'name_setup');
  };

  return (
    <div onClick={phase === 'tap' ? handleTap : undefined} style={{
      ...
      <div style={{ fontFamily:'var(--font-display)', fontSize:13, fontWeight:700, color:'rgba(245,230,200,0.4)', letterSpacing:'0.22em', textTransform:'uppercase', marginBottom:48 }}>
        Build Your Empire on Solana
      </div>

      {phase === 'loading' && (
        <div style={{ width:220, height:4, background:'rgba(255,255,255,0.06)', borderRadius:2, overflow:'hidden' }}>
          <div style={{ height:'100%', borderRadius:2, background:'linear-gradient(90deg, #9945FF, #14F195)', animation:'loading-bar 2s ease-in-out forwards' }} />
        </div>
      )}
